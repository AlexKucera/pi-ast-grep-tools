...
- **Hashline output format** — `ast_grep_search` now returns LINE#HASH|content anchors (e.g., `12#MQ|content`) compatible with @davehardy20/pi-hashline-tools' `read_hashed()` / `hashline_edit()`. Results can be copied directly into `hashline_edit()` calls without manual line number lookup.
- **Hashline engine** (`src/hashline.ts`) — FNV-1a 32-bit hash algorithm ported verbatim from @davehardy20/pi-hashline-tools. Uses a ZPMQVRWSNKTXJBYH alphabet designed to avoid hex digits, confusable characters, and English words. No external dependencies — always available.
...
- README updated with hashline compatibility table verifying byte-for-byte match against pi-hashline-tools source.
...
- Removed ESM-only constraint — hashline module no longer uses top-level `await` (no external deps).
